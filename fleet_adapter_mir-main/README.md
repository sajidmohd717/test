...
Updated fleet adapter mir under testing now. Fixing some bugs with orignal mir fleet adapter (Not officially supported anymore)
...
Originally osrf used to create a MiR workspace. But cloning this repository on our rmf workspace was a better option for our project.

```bash
# Create the MiR workspace and clone the repository
cd ~/rmf_ws/src
git clone git@github.com:JurongEast/fleet_adapter_mir.git
...
cd ~/rmf_ws/src/fleet_adapter_mir/configs
