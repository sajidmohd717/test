...
def create_fleet(config,nav_graph_path,api_yaml,task_request_check, mock):
...
def create_robot_command_handles(config, nav_graph_path, api_yaml,robot_traits, handle_data, dry_run=False):
    robots = {}
    ...
    for robot_name, robot_config in config['robots'].items():
        ...
        robot.mir_api =  MirAPI(prefix,headers,api_yaml)
...
def main(argv=sys.argv, task_request_check=None, mock=False):
    ...
    parser.add_argument('-a', '--api_file',type = str,required=True,
                        help='Path to the API_config for this fleet adapter')
    ...
    with open(api_path, 'r') as g:
        api_yaml = yaml.safe_load(g)

    print("=======api_yaml=======",api_yaml)
    ...
    # INIT FLEET ==============================================================
    adapter, fleet, fleet_name, robot_traits, nav_graph = create_fleet(config,nav_graph_path,api_yaml,task_request_check=task_request_check,
                                                                  mock=mock)

    # INIT TRANSFORMS =========================================================

    transforms = {}
    robot_map_tf = config["rmf_transform"]
    for map_name, tf in robot_map_tf.items():
        print("======tf======",tf)
        print("========tf['reference_coordinates']========",tf['reference_coordinates'])
        print("========tf['reference_coordinates']['rmf']========",tf['reference_coordinates']['rmf'])
        rmf_coordinates = tf['reference_coordinates']['rmf']
        mir_coordinates = tf['reference_coordinates']['mir']
        transform = compute_transforms(rmf_coordinates, mir_coordinates)
        transforms[tf['rmf_map_name']] = transform
    
            
    print("========robot_map_transforms=======",transforms)
    ...
    robots = create_robot_command_handles(config, nav_graph_path, api_yaml, robot_traits, handle_data, dry_run=dry_run)
